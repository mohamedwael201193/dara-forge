...
export function StorageUploadSection() {
  ...
  const [status, setStatus] = React.useState<
    "idle" | "preparing" | "registering" | "checking" | "done" | "error"
  >("idle");
  ...
  // DA State
  const [daStatus, setDaStatus] = React.useState<
  ...
  async function handleDAPublish() {
    ...
    try {
      ...
      setSuccessNotification({
        ...
        message:
          "Your dataset is now permanently available on the Data Availability network.",
      });
    } catch (error: any) {
    ...
    try {
      const { txHash, explorerUrl, activityData } = await anchorWithWallet(
        ...
        undefined, // project
        {
          ...
          description: `Storage section anchor: ${
            currentUpload?.fileName || "unknown file"
          }`,
        }
      );
      ...
      setSuccessNotification({
        ...
        message:
          "Your dataset has been permanently anchored on 0G Chain with full provenance.",
        txHash,
        explorerUrl,
      });
    } catch (e: any) {
  ...
  async function handleAttestation() {
    ...
    try {
      ...
      const response = await fetch(apiUrl("/api/attest"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-300 mt-2">
            Securely upload to 0G Storage with cryptographic proofs and
          ...
          <CardContent className="p-6 space-y-6">
            ...
            >
              <p className="text-slate-300 mb-3">
                Drag & drop files here or choose from device
              </p>
              <input
                type="file"
                multiple
              ...
              <div className="mt-4 text-sm text-slate-400">
                Selected: {files.length} file(s) • {fmtKB(total)}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div
                className={cn(
                  "rounded-lg p-3 border",
                  ["registering", "checking", "done"].includes(status)
                    ? "border-green-500 bg-green-500/10"
                    : "border-slate-700 bg-slate-800/40"
                )}
              >
                <p className="text-slate-200 text-sm font-medium">
                  Registration transaction complete
                </p>
              </div>
              <div
                className={cn(
                  "rounded-lg p-3 border",
                  ["checking", "done"].includes(status)
                    ? "border-purple-500 bg-purple-500/10"
                    : "border-slate-700 bg-slate-800/40"
                )}
              >
                <p className="text-slate-200 text-sm font-medium">
                  Waiting for checking file metadata…
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <Button
                ...
                disabled={!files.length || status === "preparing"}
              >
            ...
            {error && (
              <div className="text-red-400 text-sm">Upload failed: {error}</div>
            )}
              ...
              <div className="rounded-lg border border-slate-700 bg-slate-800/40 p-4 space-y-3">
                  ...
                  <div className="space-y-2 p-3 bg-slate-800/60 border border-slate-600 rounded-lg">
                    ...
                    <div className="text-sm space-y-2">
                      <div>
                        ...
                        <code className="ml-2 text-xs text-slate-300">
                          {daInfo.blobHash.slice(0, 16)}...
                        </code>
                      </div>
                      ...
                      {daInfo.txHash && (
                        <div className="mt-3">
                          <Button
                            ...
                            onClick={() =>
                              window.open(
                                `https://chainscan-galileo.0g.ai/tx/${daInfo.txHash}`,
                                "_blank"
                              )
                            }
                          >
                  ...
                  <div className="bg-slate-700/30 rounded-lg p-3">
                    ...
                    <div className="text-xs text-slate-400 space-y-1">
                      <p>
                        ...
                        {fmtKB(files.reduce((sum, f) => sum + f.size, 0))}
                      </p>
                ...
                <div className="bg-slate-700/50 rounded p-3 text-sm">
                  ...
                  <p className="text-slate-400">
                    • Storage Payer: Server (0xDE84...4435) - Sponsored Upload
                  </p>
                  <p className="text-slate-400">
                    • Dataset Author:{" "}
                    {attestation
                      ? attestation.address
                      : "Sign attestation to verify"}
                  </p>
                  <p className="text-slate-400">
                    • Anchor Signer: Connected Wallet (for on-chain provenance)
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {root && (
                    <>
                      <AnimatedButton
                        variant="secondary"
                        icon={Download}
                        href={`/api/storage/proxy?root=${encodeURIComponent(
                          root
                        )}&name=${encodeURIComponent(
                          files[0]?.name || "dataset"
                        )}`}
                      ...
                      <AnimatedButton
                        ...
                        href={`/api/storage/proxy?root=${encodeURIComponent(
                          root
                        )}&proof=1&name=${encodeURIComponent(
                          (files[0]?.name || "dataset") + ".proof"
                        )}`}
                  ...
                  <AnimatedButton
                    variant="info"
                    icon={Eye}
                    href={`https://storagescan-galileo.0g.ai/files?root=${encodeURIComponent(
                      root
                    )}`}
                    target="_blank"
                  >
                ...
                {showAdvanced && (
                  <div className="bg-slate-800/60 rounded border border-slate-600 p-4">
                    ...
                    <p className="text-slate-400 text-xs mt-2">
                      Note: Server anchoring shows server as uploader, not ideal
                      for provenance
                    </p>
                  </div>
                )}
