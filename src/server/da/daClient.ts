...
const DA_RPC = process.env.VITE_OG_RPC || 'https://evmrpc-testnet.0g.ai/'; // Use regular 0G RPC
...
class OGDAClient {
  ...
  async submitBlob(data: Uint8Array, metadata?: {
    datasetId?: string;
    rootHash?: string;
    ...
    if (data.length > MAX_BLOB_SIZE) {
      throw new Error(`Data too large: ${data.length} bytes (max: ${MAX_BLOB_SIZE})`);
    }
  ...
  async verifyAvailability(blobHash: string): Promise<boolean> {
    ...
    try {
      ...
      console.log('[0G DA] Searching last', searchBlocks, 'blocks from block', latestBlock);
      ...
      for (let i = 0; i < searchBlocks; i++) {
        ...
        try {
          ...
          for (const txHash of block.transactions) {
            try {
              ...
              if (cleanTxData.includes(cleanBlobHash)) {
                console.log('[0G DA] ✅ Found DA commitment in block', blockNumber);
          ...
          console.warn('[0G DA] Error reading block', blockNumber, blockError.message);
          continue;
        }
      }
      
      console.log('[0G DA] ⚠️  No DA commitment found in last', searchBlocks, 'blocks');
      return false;
