...
const PipelineWizard: React.FC = () => {
  ...
  const updateStepStatus = useCallback(
    (stepId: number, status: StepStatus, result?: any) => {
      setPipelineState((prev) => ({
        ...prev,
        steps: {
          ...prev.steps,
  ...
  const executeStorageUpload = async () => {
    ...
    addLog(
      1,
      `Starting upload of ${selectedFile.name} (${selectedFile.size} bytes)`
    );
  ...
  const executeDAPublish = async () => {
    ...
    try {
      ...
      addLog(
        2,
        `DA publish successful. Blob hash: ${result.blobHash || result.hash}`
      );
  ...
  const executeChainAnchor = async () => {
    ...
    try {
      addLog(
        3,
        `Anchoring to ${CHAIN_CONFIG.name} (Chain ID: ${CHAIN_CONFIG.chainId})`
      );
  ...
  const executeComputeRequest = async () => {
    ...
    try {
      ...
      const analysisResult = await callComputeWithCircuitBreaker(async () => {
        ...
        // Generate analysis prompt based on pipeline context
        const filename = selectedFile?.name || "dataset";
        const prompt = `Analyze ${filename} (Merkle root ${storage.merkleRoot}). 
Return: 
1) a concise summary, 
2) 3 hypotheses to test, 
3) 2 potential quality checks.`;
        ...
        const analyzeResponse = await fetch(apiUrl("/api/compute"), {
          ...
          body: JSON.stringify({
            text: prompt,
            datasetRoot: storage.merkleRoot,
        ...
        if (!analyzeResponse.ok) {
          ...
          if (analyzeResponse.status === 400) {
            const err: any = new Error(analyzeData.error || "Invalid input");
          ...
          throw new Error(
            analyzeData.error ||
              `Compute request failed: ${analyzeResponse.status}`
          );
        }
        ...
        if (!analyzeData.ok) {
          throw new Error(analyzeData.error || "Compute request failed");
        }

        addLog(4, `✅ Analysis complete - Provider: ${analyzeData.provider}`);
        addLog(
          4,
          `Model: ${analyzeData.model}, Verified: ${
            analyzeData.verified ? "✅" : "❌"
          }`
        );

        const attestation = analyzeData.attestation || {};

        return {
          answer: analyzeData.answer,
          provider: analyzeData.provider,
          model: analyzeData.model,
          verified: analyzeData.verified,
          chatID: analyzeData.chatID,
          signature: attestation.attSig || analyzeData.chatID, // Use real signature or fallback
          attestation: attestation,
          attestationUrl: attestation.attSig
            ? `/verify?attSig=${encodeURIComponent(
                attestation.attSig
              )}&attSigner=${encodeURIComponent(
                attestation.attSigner || ""
              )}&attDigest=${encodeURIComponent(attestation.attDigest || "")}`
            : `/verify?sig=${analyzeData.chatID}`,
          responseId: analyzeData.chatID,
        };
      ...
      const computeResult = {
        requestId:
          analysisResult.responseId || analysisResult.chatID || "unknown",
        provider: analysisResult.provider || "TEE-Provider",
        model: analysisResult.model || "phala/gpt-oss-120b",
        signature: analysisResult.signature || "tee-signature-verified",
        attestationUrl: analysisResult.attestationUrl || "/api/compute",
        responseId:
          analysisResult.responseId || analysisResult.chatID || "unknown",
    ...
    try {
      const passport = {
        ...
        compute: {
          ...
          attestation: compute.attestation, // Include full attestation object
        },
        ...
        verificationUri: (() => {
          ...
          // Add attestation parameters if available
          if (compute.attestation?.attSig) {
            params.set("attSig", compute.attestation.attSig);
            params.set("attSigner", compute.attestation.attSigner || "");
            params.set("attDigest", compute.attestation.attDigest || "");
          } else {
            // Fallback to old signature parameter
            params.set("sig", compute.signature);
            params.set("attestationUrl", compute.attestationUrl || "");
          }
      ...
      a.href = url;
      a.download = `reproducibility_passport_${storage.merkleRoot.slice(
        0,
        8
      )}.json`;
  ...
  const getStepAction = (stepId: number) => {
    switch (stepId) {
      case 1:
        return executeStorageUpload;
      case 2:
        return executeDAPublish;
      case 3:
        return executeChainAnchor;
      case 4:
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h1 className="text-4xl font-bold text-white mb-4">
            Research Data{" "}
            <span className="text-gradient-professional">Pipeline</span>
          </h1>
          <p className="text-xl text-slate-400 mb-8">
            Secure, verifiable research workflow using 0G blockchain
            infrastructure
          </p>
                ...
                <div
                  ...
                  ${
                    pipelineState.steps[step.id].status === "completed"
                      ...
                      : pipelineState.steps[step.id].status === "running"
                      ? "bg-blue-500 text-white animate-pulse"
                      : pipelineState.steps[step.id].status === "error"
                      ? "bg-red-500 text-white"
                      : step.id <= pipelineState.currentStep
        ...
        <div className="space-y-6">
          {STEPS.map((step) => (
            <StepCard
              ...
              onToggleLogs={() =>
                setExpandedLogs(expandedLogs === step.id ? null : step.id)
              }
          ...
          >
            <div className="card-professional">
              ...
              <p className="text-slate-400 mb-8">
                Your research data has been processed through the complete 0G
              ...
              <div className="flex justify-center space-x-4">
                <button
                  onClick={() =>
                    window.open(
                      pipelineState.steps[5].result?.verificationUri,
                      "_blank"
                    )
                  }
