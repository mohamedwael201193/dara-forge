...
function getIndexerBase(): string {
  return (import.meta.env.VITE_OG_INDEXER || "https://indexer-storage-testnet-turbo.0g.ai/").replace(/\/$/, "");
}
 ...
 */
export async function uploadFileClient(file: File, _opts?: { onProgress?: UploadProgress }) {
 ...
 */
export async function uploadDirectoryClient(files: File[], _opts?: { onProgress?: UploadProgress }) {
  const fd = new FormData();
...
export async function uploadBlobTo0GStorageViaBrowser(blob: Blob, name = 'upload.bin'): Promise<PublishResult> {
  const file = new File([blob], name);
  return uploadFileTo0GStorageViaBrowser(file);
}

export async function uploadFileTo0GStorageViaBrowser(file: File): Promise<PublishResult> {
  ...
  const tx = (data.indexerTx && typeof data.indexerTx === 'string' ? data.indexerTx : '') as Hex;
  const chainTx = `${import.meta.env.VITE_OG_EXPLORER || 'https://chainscan-galileo.0g.ai'}/tx/${tx || ''}`;
  return { rootHash: data.root as Hex, txHash: (tx || '0x') as Hex, chainTx };
}

export async function uploadBlobTo0GStorage(blob: Blob, name = 'upload.bin'): Promise<PublishResult> {
