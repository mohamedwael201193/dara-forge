...
class ComputeCircuitBreaker {
  ...
  canAttemptCall(): boolean {
    ...
    switch (this.state) {
      ...
      case 'HALF_OPEN':
        console.log('[Circuit Breaker] In HALF_OPEN state, allowing single test call');
        return true;
  ...
  setMaintenanceMode(enabled: boolean): void {
    console.log(`[Circuit Breaker] ${enabled ? 'Enabling' : 'Disabling'} maintenance mode`);
  ...
  getStatusMessage(): string {
    ...
    switch (this.state) {
      ...
      case 'OPEN':
        const retryIn = this.nextRetryTime ? Math.ceil((this.nextRetryTime - Date.now()) / 1000) : 0;
  ...
  getGracefulOptions(): {
    ...
    const now = Date.now();
    const retryEstimate = this.nextRetryTime ? Math.max(0, this.nextRetryTime - now) : 0;
...
export async function callComputeWithCircuitBreaker<T>(
  ...
  try {
    ...
    const enhancedError = new Error(
      `${error instanceof Error ? error.message : String(error)} (Circuit: ${status.state}, Failures: ${status.failureCount})`
    );
