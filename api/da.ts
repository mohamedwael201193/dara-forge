    ...
    try {
      ...
      for (let i = 0; i < searchBlocks; i++) {
        ...
        for (const txHash of block.transactions) {
          ...
          if (tx.data.includes(blobHash.slice(2))) { // Remove 0x prefix
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (action === 'submit') {
      ...
      const result = await daClient.submitBlob(new Uint8Array(blobData), metadata);
    ...
    if (error.message?.includes('insufficient funds')) {
      userMessage = 'Insufficient OG tokens for DA transaction. Please add more funds to your wallet.';
    } else if (error.message?.includes('network')) {
      userMessage = 'Network error - unable to reach 0G blockchain. Please check your connection.';
